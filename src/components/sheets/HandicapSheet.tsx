...
export function HandicapSheet() {
  ...
  const handleSave = async () => {
    ...
    if (isNaN(val) || val < 0 || val > 54) { setError('Handicap must be between 0 and 54.'); return }
    setError(null)
    setSaving(true)
    try {
    ...
    <BottomSheet isOpen={isOpen} onClose={closeSheet} title={t('sheet.hcp.title')}>
      <div style={{ padding: '16px 20px 28px' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--ink-3)', marginTop: 4 }}>Current Handicap Index</div>
        </div>

        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Update Handicap</div>
          <input
            type="number"
            inputMode="decimal"
        ...
        {success && <div style={{ fontSize: 13, color: '#2E7D32', fontWeight: 600, marginBottom: 12, textAlign: 'center' }}>Saved!</div>}
        ...
        >
          <span style={{ fontSize: 15, fontWeight: 700, color: 'white' }}>{saving ? 'Saving…' : 'Save Handicap'}</span>
        </div>
      </div>
    </BottomSheet>
  )
